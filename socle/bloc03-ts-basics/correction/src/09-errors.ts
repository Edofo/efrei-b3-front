/* Quest 9 · When it breaks — solution */
...
export function assertAge(age: unknown): number {
  // `typeof` is a type guard: after it, the compiler knows `age` is a number.
  if (typeof age !== "number" || !Number.isInteger(age) || age < 0 || age > MAX_AGE) {
...
export function averageRating(ratings: number[]): number {
  if (ratings.length === 0) throw new Error("No ratings");
...
// Challenge ⭐: an error of our own. `name` shows up in messages and logs.
...
export function findOrThrow(movies: Movie[], title: string): Movie {
  ...
  if (!movie) throw new MovieNotFoundError(`"${title}" is not in the catalogue`);
  return movie;
}
